...
function profilUser(contentPage){
    ...
    infoAdressStreet.textContent = `Tu habites ici non? (je t'enverrai des lettres, pleins d'amour...) : ${infoUser.attrs.addressStreet}`;

    const bonneUsage = document.createElement("div");
    bonneUsage.className = "infoUser";
    bonneUsage.textContent = `Merci pour toutes ces infos j'en ferai bon usage ${infoUser.attrs.firstName} ;)`;
...
function generateGraphLinear() {
    ...
    xpAmount.textContent = `Nombres de transactions d'exp reçues (projets, piscine, évaluation...) : ${transactionsEXP().length}\n`;
    ...
    for (let i = 0; i <= 9; i++) {
        if (i === 0){
            ...
            text.setAttribute("y", y); // Position verticale du texte
            text.setAttribute("fill", "black"); // Couleur du texte
            text.textContent = `Nombre d'exp total : ${sommeOfAllValues}`; // Valeur de la graduation
            svg.appendChild(text);
        } else if(i === 7){
            const y = 400 - i * 40; // Position verticale de la graduation
            const text = document.createElementNS("http://www.w3.org/2000/svg", "text");
